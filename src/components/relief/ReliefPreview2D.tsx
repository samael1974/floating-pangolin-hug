...
/**
 * Very small blur (box blur) on grayscale buffer.
 * radius: 0..3 recommended for preview.
 */
...
/**
 * Simple local contrast/detail enhancement:
 * detail = 0..1, increases (src - blurred) added back.
 */
function enhanceDetail(
  ...
  // Light blur to compute "low-frequency"
  const blurred = boxBlurGray(src, w, h, 1);
  const dst = new Uint8ClampedArray(src.length);

  const k = lerp(0.0, 1.5, detail); // strength
...
/**
 * Edge mode "sharp": increase contrast via curve + optional threshold.
 */
function applyEdgeMode(src: Uint8ClampedArray, edge: "round" | "sharp") {
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    // Optional mild thresholding to "snap" logos
    const t = c;
    dst[i] = clampByte(Math.round(t * 255));
  }
  return dst;
}

/**
 * Convert RGBA image data to grayscale heightmap (0..255).
 */
...
export default function ReliefHeightmapPreview({
  ...
  React.useEffect(() => {
    ...
    async function run() {
      ...
      if (blurRadius > 0) {
        gray = boxBlurGray(gray, w, h, blurRadius);
      }
  ...
  return (
    <div className="rounded-lg bg-wh
