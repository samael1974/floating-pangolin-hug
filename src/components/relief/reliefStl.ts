...
export function heightmapToAsciiStl(
  ...
  for (let y = 0; y < dyCount - 1; y++) {
    ...
    const top1 = [widthMm, y1, z1];
        const zTop0 = topZ(dxCount - 1, y);
    const zTop1 = topZ(dxCount - 1, y + 1);
    const zBot0 = bottomZ(dxCount - 1, y);
    const zBot1 = bottomZ(dxCount - 1, y + 1);

    const top0 = [widthMm, y0, zTop0];
    const top1 = [widthMm, y1, zTop1];
    const bot0 = [widthMm, y0, zBot0];
    const bot1 = [widthMm, y1, zBot1];
  ...
  for (let x = 0; x < dxCount - 1; x++) {
    const x0 = x * dx;
    const x1 = (x + 1) * dx;
       const zTop0 = topZ(x, 0);
    const zTop1 = topZ(x + 1, 0);
    const zBot0 = bottomZ(x, 0);
    const zBot1 = bottomZ(x + 1, 0);

    const top0 = [x0, 0, zTop0];
  ...
  for (let x = 0; x < dxCount - 1; x++) {
    const x0 = x * dx;
    const x1 = (x + 1) * dx;
      const zTop0 = topZ(x, dyCount - 1);
    const zTop1 = topZ(x + 1, dyCount - 1);
    const zBot0 = bottomZ(x, dyCount - 1);
    const zBot1 = bottomZ(x + 1, dyCount - 1);

    const top0 = [x0, heightMm, zTop0];
    const top1 = [x1, heightMm, zTop1];
    const bot0 = [x0, heightMm, zBot0];
    const bot1 = [x1, heightMm, zBot1];
